...
var getResult = function(file) {
    var outJson = JSON.stringify(file, null, 4);
    console.log(outJson);
};
...
if(require.main == module) {
	...
	.option('-u, --url <url_link>', 'url linkl')
	.parse(process.argv);
    if (program.url) {
	rest.get(program.url).on('complete', function(result, response) {
		...
		fs.writeFileSync(hfile,result);
		var checkJson1 = checkHtmlFile(hfile, program.checks);
